...
final class SecurityController extends AbstractController
{
    ...
    public function register(
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($formData['date_of_birth'] === '') {
                ...
                $parsedBirthDate = \DateTime::createFromFormat('Y-m-d', $formData['date_of_birth']);

                if (!$parsedBirthDate instanceof \DateTimeInterface) {
    ...
    public function forgotPassword(
        ...
        if ($request->isMethod('POST')) {
            ...
            if (empty($errors)) {
                ...
                if (!$user) {
                    $this->addFlash('error', 'Aucun email trouvÃ©.');
                    return $this->render('security/forgot_password.html.twig', [
                        'cooldownEndsAt' => $cooldownEndsAt,
                    ]);
                }
                ...
                try {
                    $message = (new Email())
                        ...
                        ->subject('RÃ©initialisation du mot de passe - ' . (new \DateTime())->format('d/m H:i:s'))
                        ->text(
                            "Bonjour,\n\n" .
                    ...
                    $this->addFlash('success', 'Code envoyÃ© avec succÃ¨s Ã  votre adresse email.');
                    return $this->redirectToRoute('app_reset_password');
                } catch (\Throwable $e) {
                    $this->addFlash('error', 'Erreur lors de lâ€™envoi de lâ€™email : ' . $e->getMessage());
                }
            }
    ...
    public function resetPassword(
        ...
        if ($email === '') {
            $this->addFlash('error', 'Veuillez dâ€™abord demander un code de rÃ©initialisation.');
        ...
        if ($request->isMethod('POST')) {
                ...
                if (empty($errors)) {
                    ...
                    if (!$user) {
                        ...
                        if ($otp->getExpires_at() < $now) {
                            ...
                            $this->addFlash('success', 'Code vÃ©rifiÃ© avec succÃ¨s.');
                            return $this->redirectToRoute('app_reset_password');
                        }
                    }
                }
            ...
            if ($action === 'change_password') {
                if (!$codeVerified) {
                    $this->addFlash('error', 'Veuillez vÃ©rifier votre code avant de changer le mot de passe.');
                    return $this->redirectToRoute('app_reset_password');
                }
    ...
    public function verifyLoginCode(
        ...
        if (!$userId) {
            $this->addFlash('error', 'Votre session de vÃ©rification a expirÃ©. Veuillez vous reconnecter.');
            return $this->redirectToRoute('app_login');
        }
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($code === '') {
                ...
                if (!$otp) {
                    $this->addFlash('error', 'Aucun code valide trouvÃ©.');
                } elseif ($otp->getExpires_at() < new \DateTime()) {
                    $this->addFlash('error', 'Le code a expirÃ©.');
                } elseif (!password_verify($code, $otp->getCode_hash())) {
    ...
    public function resendLoginCode(
        ...
        if (!$userId) {
            $this->addFlash('error', 'Session expirÃ©e. Veuillez vous reconnecter.');
            return $this->redirectToRoute('app_login');
        }
        ...
        $message = (new Email())
            ...
            ->to($email)
            ->subject('Nouveau code de vÃ©rification - Advisora')
            ->html("
        ...
        $this->addFlash('success', 'Un nouveau code a Ã©tÃ© envoyÃ©.');

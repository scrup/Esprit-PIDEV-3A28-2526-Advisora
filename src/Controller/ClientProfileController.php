    ...
    public function edit(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($imageFile) {
                ...
                try {
                    $imageFile->move($this->getParameter('user_images_directory'), $newFilename);
                    $user->setImage_path('uploads/users/' . $newFilename);

                    if ($oldImagePath && str_starts_with($oldImagePath, 'uploads/users/')) {
                        $oldAbsolutePath = $this->getParameter('kernel.project_dir') . '/public/' . $oldImagePath;
                        $usersDir = (string) $this->getParameter('user_images_directory');

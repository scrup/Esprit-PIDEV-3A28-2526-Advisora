 ...
 *
 * Objectif:
 * - fournir rapidement un jeu de donnees cohérent pour la demo d'analyse,
...
class ResourceBackAnalysisFixture extends Fixture implements FixtureGroupInterface
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        $statusPool = [Resource::STATUS_AVAILABLE, Resource::STATUS_RESERVED, Resource::STATUS_UNAVAILABLE];
        for ($i = 0; $i < 36; $i++) {
            $resource = new Resource();
            $resource->setNomRs(ucfirst($faker->words(2, true)));

            // Cas metier varries pour alimenter l'analyse:
